...
function MempoolPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['mempool'],
    queryFn: async () => {
      const result = await commands.getMempool();
      return result;
    },
    refetchInterval: 30000, // 30 seconds
    onSuccess: () => {
      setStatus('Mempool data fetched successfully');
    },
    onError: (err) => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-slate-400">Loading mempool...</div>
      </div>
    );
  }
