...
function AddressTxPage() {
  const [address, setAddress] = useState('');
  const setStatus = useAppStore((state) => state.setStatus);

  const { data, loading, error, execute } = useInvoke({
    onSuccess: () => {
      setStatus('Address transactions fetched successfully');
    },
    onError: (err) => {
      setStatus(`Error: ${err}`);
    },
  ...
  const handleSearch = async () => {
    ...
    await execute('get_address_transactions', { address: address.trim() });
  };
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Transactions by Address</h1>
      ...
      {data && (
        <JsonViewer data={data} title="Address Transactions" maxHeight={700} />
      )}
    </div>
  );
}

export default AddressTxPage;
